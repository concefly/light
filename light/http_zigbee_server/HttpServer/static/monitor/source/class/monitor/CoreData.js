...
qx.Class.define('monitor.CoreData',
{
	...
	members:
	{
		...
		startLongPulling: function()
		{
			this.__storeDynamic.send();
		},
		
		_onDynamic: function(e)
		{
			var dat = e.getTarget().getResponse();
			var msgList = eval('('+ dat +')');
			for(var i in msgList)
			{
				if(msgList[i].msg=='changeGroupOnline')
					this.update('GroupOnline');
				else if(msgList[i].msg=='changeGroupFixInfo')
					this.update('GroupFixInfo');
				else if(msgList[i].msg=='changeGroupVarInfo')
					this.update('GroupVarInfo');
				else if(msgList[i].msg=='changeWhiteList')
					this.update('WhiteList');
				else if(msgList[i].msg=='changeDevOnline')
					this.update('DevOnline');
				else if(msgList[i].msg=='changeDevFixInfo')
					this.update('DevFixInfo');
				else if(msgList[i].msg=='changeDevVarInfo')
					this.update('DevVarInfo');
				else if(msgList[i].msg=='changeDevDyn00')
					this.update('DevDyn00');
				else if(msgList[i].msg=='changeDevDyn01')
					this.update('DevDyn01');
			}
	...
	construct: function()
	{
		...
		//~ 初始化 store
		this.__storeDynamic = new qx.io.request.Xhr('/api/dynamic','get');
		...
		//~ 绑定 store 信号
		this.__storeDynamic.addListener('success',this._onDynamic,this);
